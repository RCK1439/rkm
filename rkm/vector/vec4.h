...
#define RKM_VEC4_FUNC_GEN(name, type)\
    ...
    RKMAPI name##_t rkm_##name##_zeros(void) {\
        return rkm_##name##_new((type)0, (type)0, (type)0, (type)0);\
    }\
    RKMAPI name##_t rkm_##name##_ones(void) {\
        return rkm_##name##_new((type)1, (type)1, (type)1, (type)1);\
    }\
    ...
    RKMAPI name##_t rkm_##name##_div(name##_t lhs, name##_t rhs) {\
        RKM_ASSERT(rhs.x != (type)0 && rhs.y != (type)0 && rhs.z != (type)0 && rhs.w != (type)0, RKM_DIVIDE_BY_ZERO);\
        return rkm_##name##_new(\
            lhs.x / rhs.x,\
            lhs.y / rhs.y,\
            lhs.z / rhs.z,\
